function control_variables = load_control_parameters()
    ...
    control_variables.piston_time_off = 0.4; % (seconds) [SELECT] Period the piston is off
    ...
    control_variables.c1_n_ticks_before_turn = 33; %[SELECT] number of ticks before start turning
    control_variables.c1_total_time_turning = 0.35; %[SELECT] seconds while robot will be turning at max rate
    ...
    control_variables.c2_n_ticks_before_turn = 30 %[SELECT] number of ticks before start turning
    control_variables.c2_total_time_turning = 0.5; %[SELECT] seconds while robot will be turning at max rate
    control_variables.c2_Kp = .5; % [SELECT] causes the robot to point toward the channel
